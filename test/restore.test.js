...
import { restorePage } from '../lib/restore.js';

test('basic restore page', async (t) => {
    const RECORD = JSON.parse(
        fs.readFileSync(new URL('./fixtures/snapshot_example.com.json', import.meta.url), 'utf-8'),
    );
    ...
    t.true(html.includes('<h1>Example Domain</h1>'));
    t.true(html.includes('<title>Example Domain</title>'));
